...
ocr_enabled := 1
ocr_language := ""
for k,v in {"Windows.Globalization.Language":"{9B0252AC-0C27-44F8-B792-9793FB66C63E}", "Windows.Graphics.Imaging.BitmapDecoder":"{438CCB26-BCEF-4E95-BAD6-23A822E58D01}", "Windows.Media.Ocr.OcrEngine":"{5BFFA85A-3384-3540-9940-699120D428A8}"}
{
	CreateHString(k, hString)
	VarSetCapacity(GUID, 16), DllCall("ole32\CLSIDFromString", "wstr", v, "ptr", &GUID)
...
if (ocr_enabled = 1)
{
	...
	for k,v in ["ko","en-"] ; priority list
	{
		Loop, Parse, list, `n, `r
		{
			if (InStr(A_LoopField, v) = 1)
			{
	...
	if (ocr_language = "")
		if ((ocr_language := SubStr(list, 1, InStr(list, "`n")-1)) = "")
			msgbox, 0x1030, WARNING!!, % "No OCR supporting languages are installed on your system! Please follow the Knowledge Base guide to install a supported language as a secondary language on Windows."
}
...
buff_values := {}
for k,v in ["haste","melody","redboost","blueboost","whiteboost","focus","bombcombo","balloonaura","clock","jbshare","babylove","inspire","bear","pollenmark","honeymark","festivemark","popstar","comforting","motivating","satisfying","refreshing","invigorating","blessing","bloat","guiding","mondo","reindeerfetch","tideblessing"]
	buff_values[v] := {}
...
; status_changes format: A_Min*60+A_Sec:status_number (0 = other, 1 = gathering, 2 = converting)
status_changes := {}
...
; FORM MESSAGE
message := "Hourly Reports will start sending in **" time "**\n"
...
regions := {"honey/sec":[120,120,4080,1080]
	, "stats":[w-1560-120,120,1560,h-240]
	, "backpack":[120,240+1080,4080,678]
	, "buffs":[120,360+1758,4080,h-480-1758]}
	
stat_regions := {"lasthour":[regions["stats"][1]+100,regions["stats"][2]+100,regions["stats"][3]-200,1206]
...
graph_regions := {}
graph_regions["honey/sec"] := [regions["honey/sec"][1]+320,regions["honey/sec"][2]+130,3600,800]
graph_regions["backpack"] := [regions["backpack"][1]+320,regions["backpack"][2]+130,3600,400]
graph_regions["boost"] := [regions["buffs"][1]+320,regions["buffs"][2]+135,3600,280]
graph_regions["haste"] := [regions["buffs"][1]+320,regions["buffs"][2]+435,3600,280]
graph_regions["focus"] := [regions["buffs"][1]+320,regions["buffs"][2]+735,3600,280]
graph_regions["bombcombo"] := [regions["buffs"][1]+320,regions["buffs"][2]+1035,3600,280]
graph_regions["balloonaura"] := [regions["buffs"][1]+320,regions["buffs"][2]+1335,3600,280]
graph_regions["inspire"] := [regions["buffs"][1]+320,regions["buffs"][2]+1635,3600,280]
graph_regions["reindeerfetch"] := [regions["buffs"][1]+320,regions["buffs"][2]+1935,3600,280]
graph_regions["honeymark"] := [regions["buffs"][1]+320,regions["buffs"][2]+2235,3600,120]
graph_regions["pollenmark"] := [regions["buffs"][1]+320,regions["buffs"][2]+2375,3600,120]
graph_regions["festivemark"] := [regions["buffs"][1]+320,regions["buffs"][2]+2515,3600,120]
graph_regions["popstar"] := [regions["buffs"][1]+320,regions["buffs"][2]+2655,3600,110]
graph_regions["melody"] := [regions["buffs"][1]+320,regions["buffs"][2]+2785,3600,110]
graph_regions["bear"] := [regions["buffs"][1]+320,regions["buffs"][2]+2915,3600,110]
graph_regions["babylove"] := [regions["buffs"][1]+320,regions["buffs"][2]+3045,3600,110]
graph_regions["jbshare"] := [regions["buffs"][1]+320,regions["buffs"][2]+3175,3600,110]
graph_regions["guiding"] := [regions["buffs"][1]+320,regions["buffs"][2]+3305,3600,110]
graph_regions["honey"] := [stat_regions["lasthour"][1]+200,stat_regions["lasthour"][2]+650,1080,480]
graph_regions["honey12h"] := [stat_regions["session"][1]+200,stat_regions["session"][2]+734,1080,480]
...
status_changes := {0:2,180:1,780:2,1080:1,1680:2,1784:3,1832:2,1980:1,2100:3,2120:1,2580:2,2880:1,3480:2}
...
for k,v in buff_values
{
	Loop, 601
	{
		Random, x, 0, (k = "redboost" || k = "whiteboost" || k = "precision") ? 2 : 10
		x := (x > 6) ? 10 : x
		v[A_Index-1] := Abs(x-v[A_Index-2]) > 4 ? 10 : x
	}
}
...
/*
DetectBuffs()
if roblox is open, this function detects buffs in BSS and updates the relevant arrays with current buff values
this function returns a list of buffs and their values delimited by new lines
*/
...
DetectBuffs()
{
	...
	{
		if (Gdip_ImageSearch(pBMArea, buff_bitmaps["pBM" v], , , 30, , , InStr(v, "mark") ? 6 : (v = "guiding") ? 10 : 0, , 7) = 1)
			buff_values[v][i] := 1
		else
			buff_values[v][i] := 0
	}
	...
	{
		if (Gdip_ImageSearch(pBMArea, buff_bitmaps["pBMHaste"], list, x, 30, , , , , 6) = 0)
			break
		
		x := SubStr(list, 1, InStr(list, ",")-1)
		
		if ((s := Gdip_ImageSearch(pBMArea, buff_bitmaps["pBMMelody"], , x+2, 15, x+34, 40, 12)) = 0)
		{
			if !buff_values["haste"][i]
			{
				Loop, 9
				{
	...
	Loop, 3
	{
		if (Gdip_ImageSearch(pBMArea, buff_bitmaps["pBMBoost"], list, , 30, x, , , , 7) = 0)
			break
	...
	for k,v in ["redboost","blueboost","whiteboost"]
		if !buff_values[v][i]
			buff_values[v][i] := 0
	
	; 2 digit
	for k,v in ["blessing","inspire"]
	{
		if (Gdip_ImageSearch(pBMArea, buff_bitmaps["pBM" v], list, , 20, , , (v = "blessing") ? 21 : 0, , (v = "blessing") ? 6 : 7) = 0)
		{
			buff_values[v][i] := 0
			continue
		}
	...
	; scaled
	for k,v in ["bloat","comforting","motivating","satisfying","refreshing","invigorating"]
	{
		if (Gdip_ImageSearch(pBMArea, buff_bitmaps["pBM" v], list, , 30, , , , , 6) = 0)
		{
			buff_values[v][i] := 0
			continue
		}
		...
		Gdip_ImageSearch(pBMArea, buff_bitmaps["pBM" v], list, x, 6, x+38, 44)
		
		y := SubStr(list, InStr(list, ",")+1)
		
		buff_values[v][i] :=  Round(Min((44 - y) / 38, 1) * ((v = "bloat") ? 5 : 100) + ((v = "bloat") ? 1 : 0), (v = "bloat") ? 2 : 0)
	}
	
	; tide
	for k,v in ["tideblessing"]
	{
		if (Gdip_ImageSearch(pBMArea, buff_bitmaps["pBM" v], list, , 30, , , , , 6) = 0)
			continue
		...
		Gdip_ImageSearch(pBMArea, pBM, list, x-16, 6, x+36, 44)
		Gdip_DisposeImage(pBM)
		
		y := SubStr(list, InStr(list, ",")+1)
		buff_values[v][i] := Round(1.01 + 0.19 * (44.3 - y) / 38, 2)
	}
...
/*
DetectHoney()
if roblox is open, this function uses OCR to detect the current honey value in BSS
as well as updating the relevant honey array, this function returns the current honey value
function is also a WIP, although a lot of work has gone into preventing misreadings
will be reworked when Onett decides to abbreviate honey at the top, which could remove the OCR dependency
*/
...
SendHourlyReport()
{
	...
	if (honey_values.Count() > 0)
	{
		...
		Loop, Parse, str, %A_Space%
			honey_values.Delete(A_LoopField)
		
		min_value := minX(honey_values), max_value := Max(maxX(honey_values), min_value+1000), value_range := max_value - min_value
	}
	else
		min_value := 0, max_value := 1000, value_range := 1000
	
	; populate honey_values array, fill missing values
	enum := ObjNewEnum(honey_values)
	enum.Next(x2,y2)
	for x1,y1 in honey_values
	{
		if (enum.Next(x2,y2) = 0)
		{
			if (x1 < 60)
				Loop % (60 - x1)
	...
	{
		FormatTime, hhmm, %time%, HH:mm
		times_12h.InsertAt(1, hhmm)
		EnvAdd, time, -2, h
	}
	
	; create report bitmap and graphics
	pBMReport := Gdip_CloneBitmapArea(pBM)
	G := Gdip_GraphicsFromImage(pBMReport)
	Gdip_SetSmoothingMode(G, 4)
	Gdip_SetInterpolationMode(G, 7)
	
	; set variable graph bounds
	min_gradient := 0, max_gradient := Max(maxX(honey_gradients), min_gradient+1000), gradient_range := max_gradient - min_gradient
	min_12h := minX(honey_12h), max_12h := Max(maxX(honey_12h), min_12h+1000), 12h_range := max_12h - min_12h

	; draw times
	for k,v in ["honey/sec","backpack","buffs"]
	{
		Loop, 7
		{
			x := regions[v][1]+320+(regions[v][3]-480)*(A_Index-1)//6
			Gdip_TextToGraphics(G, times[A_Index], "s44 Center Bold cffffffff x" x " y" regions[v][2]+regions[v][4]-85, "Segoe UI")
		}
	}
	for k,v in ["honey","honey12h"]
	{
		source := (A_Index = 1) ? "times" : "times_12h"
		Loop, 7
		{
			x := graph_regions[v][1]+graph_regions[v][3]*(A_Index-1)//6
			Gdip_TextToGraphics(G, %source%[A_Index], "s30 Center Bold cffffffff x" x " y" graph_regions[v][2]+graph_regions[v][4]+14, "Segoe UI")
		}
	}
	...
	for k,v in graph_regions
	{
		...
		switch k
		{
			case "honey/sec":
			Loop, 5
				Gdip_TextToGraphics(G, FormatNumber(max_gradient-(gradient_range*(A_Index-1))//4), "s40 Right Bold cffffffff x" v[1]-320 " y" v[2]+v[4]*(A_Index-1)//4-28, "Segoe UI", 240)
			...
			for i,j in status_changes
			{
				...
				points.Push(4+i*v[3]/3600, 4+v[4]-(honey_gradients[(i+30)//60]+((i+30)/60-(i+30)//60)*(honey_gradients[(i+30)//60+1]-honey_gradients[(i+30)//60])-min_gradient)/gradient_range*v[4])
				for x,y in honey_gradients
					((y != "") && (x >= (i+30)/60 && x <= (m+30)/60)) ? points.Push(4+(x-0.5)*v[3]/60, 4+v[4]-((y > 0) ? (((y-min_gradient)/gradient_range)*v[4]) : 0))
				points.Push(4+m*v[3]/3600, 4+v[4]-(honey_gradients[(m+30)//60]+((m+30)/60-(m+30)//60)*(honey_gradients[(m+30)//60+1]-honey_gradients[(m+30)//60])-min_gradient)/gradient_range*v[4])
			...
			case "honey":
			Loop, 5
				Gdip_TextToGraphics(G, FormatNumber(max_value-(value_range*(A_Index-1))//4), "s28 Right Bold cffffffff x" v[1] - 310 " y" v[2]+v[4]*(A_Index-1)//4 - 20, "Segoe UI", 240)
			...
			for i,j in status_changes
			{
				...
				points := []
				points.Push(4+i*v[3]/3600, 4+v[4])
				points.Push(4+i*v[3]/3600, 4+v[4]-(honey_values[i//60]+(i/60-i//60)*(honey_values[i//60+1]-honey_values[i//60])-min_value)/value_range*v[4])
				for x,y in honey_values
					((y != "") && (x >= i/60 && x <= m/60)) ? points.Push(4+x*v[3]/60, 4+v[4]-((y > 0) ? (((y-min_value)/value_range)*v[4]) : 0))
				points.Push(4+m*v[3]/3600, 4+v[4]-(honey_values[m//60]+(m/60-m//60)*(honey_values[m//60+1]-honey_values[m//60])-min_value)/value_range*v[4])
				points.Push(4+m*v[3]/3600, 4+v[4])
			...
			case "honey12h":
			Loop, 5
				Gdip_TextToGraphics(G, FormatNumber(max_12h-(12h_range*(A_Index-1))//4), "s28 Right Bold cffffffff x" v[1]-310 " y" v[2]+v[4]*(A_Index-1)//4-20, "Segoe UI", 240)
			
			points := []
			points.Push(4+v[3]*(honey_12h.MinIndex())//180, 4+v[4])
			for x,y in honey_12h
				(y != "") ? points.Push(4+v[3]*x//180, 4+v[4]-((y-min_12h)/12h_range)*v[4])
			points.Push(4+v[3]*(honey_12h.MaxIndex())//180, 4+v[4])
			color := 0xff0e8bf0
			...
			points := []
			points.Push(4+backpack_values.MinIndex()*v[3]/3600, 4+v[4])
			for x,y in backpack_values
				(y != "") ? points.Push(4+x*v[3]//3600, 4+v[4]-(y/100)*v[4])
			points.Push(4+backpack_values.MaxIndex()*v[3]/3600, 4+v[4])
			...
			Loop, 3
			{
				...
				points.Push(4+v[3]*(buff_values[i].MinIndex())//600, 4+v[4])
				for x,y in buff_values[i]
					points.Push(4+v[3]*x//600, 4+v[4]-((y <= 10) ? (y/10)*(v[4]) : 10))
				points.Push(4+v[3]*(buff_values[i].MaxIndex())//600, 4+v[4])
			...
			default:
			max := (k = "inspire") ? Max(ceil(maxX(buff_values[k])/5)*5, 5) : 10
			Gdip_TextToGraphics(G, "x0-" max, "s44 Center Bold cffffffff x" v[1]-190 " y" v[2]+190, "Segoe UI")
			...
			points.Push(4+v[3]*(buff_values[k].MinIndex())//600, 4+v[4])
			for x,y in buff_values[k]
				points.Push(4+v[3]*x//600, 4+v[4]-(y/max)*(v[4]))
			points.Push(4+v[3]*(buff_values[k].MaxIndex())//600, 4+v[4])
	...
	unix_now := SubStr(A_NowUTC, 1, 12) . "0000"
    EnvSub, unix_now, 19700101000000, Seconds
	...
	for i,j in status_list
	{
		...
		Gdip_TextToGraphics(G, hour_%j%_time_F, "s48 Left Bold cefffffff x" stat_regions["lasthour"][1]+stat_regions["lasthour"][3]//2+135 " y" stat_regions["lasthour"][2]+335+(A_Index-1)*88, "Segoe UI")
	...
	for i,j in status_list
	{
		...
		Gdip_TextToGraphics(G, %j%_time_F, "s48 Left Bold cefffffff x" stat_regions["session"][1]+stat_regions["session"][3]//2+135 " y" stat_regions["session"][2]+419+(A_Index-1)*88, "Segoe UI")
	...
	for k,v in ["clock","blessing","bloat","tideblessing","mondo"]
	{
		...
		Loop, 601
		{
			if buff_values[v][601-A_Index]
			{
				...
				pPath := Gdip_CreatePath(), pBrush := Gdip_BrushCreateSolid(0xffffffff), pPen := Gdip_CreatePen(0xff000000, 10)
				Gdip_AddPathStringSimplified(pPath, "x" buff_values[v][601-A_Index], "Segoe UI", 72, 1, stat_regions["buffs"][1]+48+(i-1)*(stat_regions["buffs"][3]-96-220)/4, stat_regions["buffs"][2]+254, 220, 90, 2)
				Gdip_DrawPath(G, pPen, pPath), Gdip_FillPath(G, pBrush, pPath)
				Gdip_DeletePen(pPen), Gdip_DeleteBrush(pBrush), Gdip_DeletePath(pPath)
				break
			}
			if (A_Index = 601)
			{
				...
				pPath := Gdip_CreatePath(), pBrush := Gdip_BrushCreateSolid(0xffffffff), pPen := Gdip_CreatePen(0xff000000, 10)
				Gdip_AddPathStringSimplified(pPath, "x0", "Segoe UI", 72, 1, stat_regions["buffs"][1]+48+(i-1)*(stat_regions["buffs"][3]-96-220)/4, stat_regions["buffs"][2]+254, 220, 90, 2)
				Gdip_DrawPath(G, pPen, pPath), Gdip_FillPath(G, pBrush, pPath)
				Gdip_DeletePen(pPen), Gdip_DeleteBrush(pBrush), Gdip_DeletePath(pPath)
			}
		}
	...
	VarSetCapacity(DateStr, 512, 0), DllCall("GetDateFormatEx", "Ptr", 0, "UInt", 0, "Ptr", 0, "Str", StrReplace(StrReplace(StrReplace(StrReplace(FormatStr, ", dddd"), "dddd, "), " dddd"), "dddd "), "Str", DateStr, "Int", 512, "Ptr", 0)
	pos := Gdip_TextToGraphics(G, times[1] " - " times[7] " • " DateStr, "s56 Center Bold c00ffffff x" stat_regions["info"][1]+stat_regions["info"][3]//2 " y" y, "Segoe UI")
	x := SubStr(pos, 1, InStr(pos, "|", , , 1)-1)
	...
	Gdip_TextToGraphics(G, DateStr, "s56 Left Bold cffffda3d x" x " y" y, "Segoe UI")
	...
	honey_values.Delete(honey_values.MinIndex(), honey_values.MaxIndex())
	honey_values[0] := current_honey
	; reset backpack values array
	current_backpack := backpack_values[backpack_values.MaxIndex()]
	backpack_values.Delete(backpack_values.MinIndex(), backpack_values.MaxIndex())
	backpack_values[0] := current_backpack
	; reset status changes array
	current_status := status_changes[status_changes.MaxIndex()]
	status_changes.Delete(status_changes.MinIndex(), status_changes.MaxIndex())
	status_changes[0] := current_status
	; reset buff values array
	for k,v in buff_values
		v.Delete(v.MinIndex(), v.MaxIndex())
}


/*
FormatNumber()
this function rounds a number 'n' to 4 s.f. and abbreviates it with common large number prefixes
...
/*
SetAbility()
this function receives messages from the background script to update ability values (pop/scorch star, etc.)
wParam is the ability (buff) to be changed, lParam is the value
*/
...
/*
SetBackpack()
this function receives messages from the background script to set the current backpack percent (filtered)
wParam is the backpack percent, lParam is the second of the hour
*/
SetBackpack(wParam, lParam){
	global backpack_values
	if (lParam > backpack_values.MaxIndex())
...
/*
minX() and maxX()
these functions return the minimum and maximum values in linear arrays 
this allows the script to mathematically obtain graph bounds using min and max x,y values
modified versions of functions by FanaticGuru, https://www.autohotkey.com/boards/viewtopic.php?t=40898
*/
