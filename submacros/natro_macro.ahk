...
nm_BitterberryFeeder(*)
{
	...
	if !ocr_enabled {
		MsgBox "OCR is not available on this system. Mutation detection will be disabled.``nThe feeder will prompt for every successful mutation.", "Bitterberry Auto-Mutator", 0x40030
	}
	; Get OCR language
	if ocr_enabled {
		...
		if (ocr_language = "")
			if ((ocr_language := SubStr(list, 1, InStr(list, "``n")-1)) = "") {
				MsgBox "No OCR supporting languages are installed. Mutation detection will be disabled.", "Bitterberry Auto-Mutator", 0x40030
				ocr_enabled := 0
			}
	}
	...
	if IsInteger(bitterberrynos) {
		if (bitterberrynos > 30)
			if (MsgBox("You have entered " bitterberrynos " which is more than 30.``nAre you sure?", "Bitterberry Auto-Mutator", 0x40034) = "No")
				ExitApp
	} else {
		MsgBox "You must enter a number for Bitterberries!!``nStopping Feeder!", "Bitterberry Auto-Mutator", 0x40010
		ExitApp
	}

	if (MsgBox("After dismissing this message,``nleft click ONLY once on BEE SLOT", "Bitterberry Auto-Mutator", 0x40001) = "Cancel")
		ExitApp
	...
	Loop
	{
		...
		{
			MsgBox "You ran out of Bitterberries!", "Bitterberry Auto-Mutator", 0x40010
			break
		}
		GetRobloxClientPos(hwnd)
		...
		if (Gdip_ImageSearch(pBMScreen, bitmaps["greensuccess"], , , , , , 20) = 1) {
			...
			; Check for Movespeed or Ability mutation using OCR
			mutationFound := ""
			if ocr_enabled {
				Sleep 200 ; Wait for mutation text to fully appear
				GetRobloxClientPos(hwnd)
				; Capture mutation text region (same coordinates as blc_mutations)
				pBitmap := Gdip_BitmapFromScreen(windowX + Round(0.5 * windowWidth - 320) "|" windowY + offsetY + Round(0.4 * windowHeight + 17) "|210|90")
				...
				; Check for Movespeed or Ability triggers
				for i, mutation in mutationTriggers {
					for j, trigger in mutation.triggers {
						if InStr(text, trigger) {
							mutationFound := mutation.name
							break 2
						}
					}
				}
			...
			if (mutationFound != "" || !ocr_enabled) {
				; Found Movespeed or Ability, or OCR disabled - prompt user
				promptMsg := ocr_enabled ? ("SUCCESS! Found " mutationFound " mutation!``nKeep this?") : "SUCCESS!!!!``nKeep this?"
				if (MsgBox(promptMsg, "Bitterberry Auto-Mutator", 0x40024) = "Yes")
					break
				else
				{
					ActivateRoblox()
			...
			; If mutation not Movespeed/Ability, continue feeding automatically (close dialog)
			else {
				ActivateRoblox()
				SendEvent "{Click " windowX + (windowWidth//2 - 132) " " windowY + offsetY + ((4*windowHeight)//10 - 150) "}" ; Close Bee
	...
	; OCR Helper Functions (from blc_mutations methodology)
