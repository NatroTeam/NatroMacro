...
nm_BitterberryFeeder(*)
{
	...
	if !ocr_enabled {
		MsgBox "OCR is not available on this system. Mutation detection will be disabled.``nThe feeder will prompt for every successful mutation.", "Bitterberry Auto-Feeder v0.2", 0x40030
	}
	; Get OCR language
	if ocr_enabled {
		...
		if (ocr_language = "")
			if ((ocr_language := SubStr(list, 1, InStr(list, "``n")-1)) = "") {
				MsgBox "No OCR supporting languages are installed. Mutation detection will be disabled.", "Bitterberry Auto-Feeder v0.2", 0x40030
				ocr_enabled := 0
			}
	}

	if (MsgBox("BITTERBERRY AUTO FEEDER v0.2 by anniespony#8135``nMake sure BEE SLOT TO MUTATE is always visible``nDO NOT MOVE THE SCREEN OR RESIZE WINDOW FROM NOW ON.``nMAKE SURE BEE IS RADIOACTIVE AT ALL TIMES!``n``n" (ocr_enabled ? "Mutation detection ENABLED: Will auto-feed until Movespeed or Ability mutation is found." : "Mutation detection DISABLED: Will prompt for every successful mutation."), "Bitterberry Auto-Feeder v0.2", 0x40001) = "Cancel")
		ExitApp
	...
	Loop
	{
		...
		if (Gdip_ImageSearch(pBMScreen, bitmaps["greensuccess"], , , , , , 20) = 1) {
			...
			if (mutationFound != "" || !ocr_enabled) {
				...
				if (MsgBox(promptMsg, "Bitterberry Auto-Feeder v0.2", 0x40024) = "Yes")
					break
				else
				{
					ActivateRoblox()
